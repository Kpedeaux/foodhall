...
-- ── vendor_terms (date-versioned plan history) ──────────────
-- Each vendor has one or more vendor_terms rows. The plan effective for any
-- given week is the row with the latest effective_from <= week_start. Adding
-- a new row never changes APPROVED weeks (those are immutable), but draft
-- weeks pick up the change on next recalc.
...
CREATE TABLE IF NOT EXISTS users (
  id                    INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  -- market_id is nullable: super_admin users are not market-scoped.
  -- A row-level CHECK below enforces that non-super-admins have a market.
  ...
  created_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
  -- Role/market_id consistency: super admins are unscoped (NULL market_id);
  -- admins and vendors must belong to a market.
  CONSTRAINT users_market_role_consistency CHECK (
